...
- (NSString*)gitPath;
{
    NSArray *searchPaths = [NSArray arrayWithObjects:@"/opt/local/bin/git",
                                                     @"/usr/local/bin/git",
                                                     @"/usr/bin/git",
                                                     nil];
    NSString *path = [[SCMIcons sharedInstance] pathForVariable:@"TM_GIT" paths:searchPaths];
    ...
    {
        {
            ...
            if (data) {
                ...
                if (r.location != NSNotFound) {
                    projectStatus |= SCMIconsStatusAhead;
                }

                r = [string rangeOfString:@"behind" options:NSCaseInsensitiveSearch];
                if (r.location != NSNotFound) {
                    projectStatus |= SCMIconsStatusBehind;
                }
            }
        }
        ...
        [task setCurrentDirectoryPath:path];
        if (path) {
            [task setArguments:[NSArray arrayWithObjects:@"ls-files", @"--exclude-standard",
                                                         @"-z", @"-t", @"-m", @"-c", @"-d", @"-o", path, nil]];
        } else {
            [task setArguments:[NSArray arrayWithObjects:@"ls-files", @"--exclude-standard",
                                                         @"-z", @"-t", @"-m", @"-c", @"-d", @"-o", nil]];
        }
