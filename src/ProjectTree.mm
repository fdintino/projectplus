...
- (void)expandItems:(NSArray*)items
      inOutlineView:(NSOutlineView*)outlineView
            toState:(NSDictionary*)treeState
{
    ...
    for (unsigned int index = 0; index < itemCount; index += 1) {
        ...
        if (NSDictionary *treeItem = [treeState objectForKey:[item objectForKey:@"displayName"]]) {
            ...
            if ([[treeItem objectForKey:@"subItems"] count]) {
                [self expandItems:[item objectForKey:@"children"]
                    inOutlineView:outlineView
                          toState:[treeItem objectForKey:@"subItems"]];
            }
        }
    }
}
...
- (NSDictionary*)outlineView:(NSOutlineView*)outlineView stateForItems:(NSArray*)items
{
    ...
    for (unsigned int index = 0; index < itemCount; index += 1) {
        ...
        if ([outlineView isItemExpanded:item]) {
            NSDictionary *subTreeState = [self outlineView:outlineView
                                             stateForItems:[item objectForKey:@"children"]];
            [treeState setObject:[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithBool:YES], @"isExpanded",
                                                                            subTreeState, @"subItems", nil]
                          forKey:[item objectForKey:@"displayName"]];
        }
    }
...
- (BOOL)ProjectTree_writeToFile:(NSString*)fileName
{
    ...
    if (result && [ProjectTree preserveTreeState]) {
        ...
        NSDictionary *treeState = [self outlineView:[self valueForKey:@"outlineView"]
                                      stateForItems:[self valueForKey:@"rootItems"]];
...
- (void)ProjectTree_outlineView:(id)outlineView
                willDisplayCell:(id)cell
                 forTableColumn:(id)column
...
+ (void)load
{
    ...
    Class OakDocumentController = NSClassFromString(@"OakDocumentController");
    Class OakProjectController  = NSClassFromString(@"OakProjectController");
    
    [OakDocumentController jr_swizzleMethod:@selector(windowDidLoad)
                                 withMethod:@selector(Document_windowDidLoad)
                                      error:NULL];

    [OakProjectController jr_swizzleMethod:@selector(windowDidLoad)
                                withMethod:@selector(ProjectTree_windowDidLoad)
                                     error:NULL];

    [OakProjectController jr_swizzleMethod:@selector(writeToFile:)
                                withMethod:@selector(ProjectTree_writeToFile:)
                                     error:NULL];

    [OakProjectController jr_swizzleMethod:@selector(outlineView:willDisplayCell:forTableColumn:item:)
                                withMethod:@selector(ProjectTree_outlineView:willDisplayCell:forTableColumn:item:)
                                     error:NULL];

    // toggle workspace vs. tabs
    if ([ProjectTree useWorkspace]) {
        [OakProjectController jr_swizzleMethod:@selector(tabBarView:didOpenTab:)
                                    withMethod:@selector(ProjectTree_tabBarView:didOpenTab:)
                                         error:NULL];

        [OakProjectController jr_swizzleMethod:@selector(tabBarView:didCloseTab:)
                                    withMethod:@selector(ProjectTree_tabBarView:didCloseTab:)
                                         error:NULL];

        [OakProjectController jr_swizzleMethod:@selector(tabBarView:didSelectTab:)
                                    withMethod:@selector(ProjectTree_tabBarView:didSelectTab:)
                                         error:NULL];
    } else {
