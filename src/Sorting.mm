                    ...
                    foldersOnTop:(BOOL)foldersOnTop
{
    ...
    for (unsigned int index = 0; index < itemCount; index += 1) {
        ...
        if (children != nil && [outlineView isItemExpanded:item] && [children count] > 0) {
...
- (void)ProjectPlus_Sorting_outlineViewItemDidExpand:(NSNotification *)notification
{
    ...
    if (children != nil && [children count] > 0) {
        ...
        @try {
            selectedItems = [self performSelector:@selector(selectedItems)];
...
{
    NSMutableDictionary* sortDescriptor = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                             [NSNumber numberWithBool:NO], @"descending",
                                             [NSNumber numberWithBool:NO], @"byExtension",
                                             [NSNumber numberWithBool:NO], @"foldersOnTop", nil];
